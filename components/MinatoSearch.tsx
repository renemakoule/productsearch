...
export default function Component() {
  ...
  return (
    <div className="min-h-screen font-sans">
      ...
      <main className={`flex flex-col items-center justify-center w-full ${showResults ? 'mt-20' : 'mt-36'} px-4`}>
        {!showResults && (
          <div className="text-center space-y-2 mb-8">
            <span className="px-3 py-1 bg-zinc-800 text-xs text-white rounded-full">
              AI product search
            </span>
            <h1 className={`text-5xl font-bold max-w-4xl leading-tight bg-clip-text text-transparent bg-gradient-to-r ${gradient}`}>
        ...
        <div className={`w-full ${showResults ? 'max-w-full' : 'max-w-xl'} space-y-4`}>
          ...
          {!showResults && (
            <div className="flex flex-wrap justify-center gap-4">
              <button onClick={handleSuggestionClick} className="flex items-center space-x-2 px-4 py-2 bg-zinc-800 rounded-md text-sm text-white font-medium z-10">
        ...
        {showResults && <TableauDeBordProduit imageUrl={""} productName={""} price={""} source={""} link={""} />}
      </main>
    </div>
  )
}